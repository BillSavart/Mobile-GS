...
renderTileAtomicCUDA(
    const uint2* __restrict__ ranges,           // 输入：图块对应的点索引范围 [start, end]
    const uint32_t* __restrict__ point_list,    // 输入：点索引列表（不排序也没关系）
    int W, int H,
    ...
    float* __restrict__ w_fg,                   // 输出：累加权重 Sum(alpha * weight)
    float* __restrict__ Ts,                     // 输出：累加透射率项 Sum(log(1-alpha))
    const float* __restrict__ bg_color,
    float* __restrict__ out_color               // 输出：累加颜色 Sum(C * alpha * weight)
)
{
    // 1. 初始化定位
    ...
    uint32_t local_id = block.thread_rank();
    bool inside = pix.x < W && pix.y < H;

    // 2. 申请 Shared Memory
    // A. 像素累加器 (256 线程对应 16x16 Tile)
    __shared__ float s_color[CHANNELS][BLOCK_X * BLOCK_Y];
    __shared__ float s_w_fg[BLOCK_X * BLOCK_Y];
    __shared__ float s_Ts[BLOCK_X * BLOCK_Y];

    // B. 点数据批次缓存 (协作式读取缓冲区)
    __shared__ int    shared_id[BLOCK_X * BLOCK_Y];
    __shared__ float2 shared_xy[BLOCK_X * BLOCK_Y];
    __shared__ float4 shared_conic[BLOCK_X * BLOCK_Y];
    __shared__ float  shared_depth[BLOCK_X * BLOCK_Y];
    __shared__ float  shared_phi[BLOCK_X * BLOCK_Y];
    __shared__ glm::vec3 shared_scale[BLOCK_X * BLOCK_Y];

    // 初始化像素缓冲区
    for (int ch = 0; ch < CHANNELS; ch++) s_color[ch][local_id] = 0.0f;
    s_w_fg[local_id] = 0.0f;
    s_Ts[local_id] = 0.0f;
    block.sync();
    ...
    const int rounds = (num_points + BLOCK_SIZE - 1) / BLOCK_SIZE;

    // 4. 外层循环：批次搬运数据 (Collective Fetching)
    for (int i = 0; i < rounds; i++)
    {
        // 协作式读取：每个线程搬运一个点的数据到 SMem
        int progress = i * BLOCK_SIZE + local_id;
        if (range.x + progress < range.y)
        {
            int coll_id = point_list[range.x + progress];
            shared_id[local_id] = coll_id;
            shared_xy[local_id] = points_xy_image[coll_id];
            shared_conic[local_id] = conic_opacity[coll_id];
            shared_depth[local_id] = depths[coll_id];
            shared_phi[local_id] = phis[coll_id];
            shared_scale[local_id] = scales[coll_id];
        }
        block.sync(); // 确保整批点都搬运到位

        // 5. 内层循环：渲染这一批次点 (SMem 读写)
        int toDo = min(BLOCK_SIZE, num_points - i * BLOCK_SIZE);
        for (int j = 0; j < toDo; j++)
        {
            float2 xy = shared_xy[j];
            float2 d_vec = { xy.x - (float)pix.x, xy.y - (float)pix.y };
            float4 con_o = shared_conic[j];

            float power = -0.5f * (con_o.x * d_vec.x * d_vec.x + con_o.z * d_vec.y * d_vec.y) - con_o.y * d_vec.x * d_vec.y;
            if (power > 0.0f) continue;

            float alpha = fminf(0.99f, con_o.w * __expf(power));
            if (alpha < 1.0f / 255.0f) continue;

            float d = fmaxf(shared_depth[j], 1e-6f);
            glm::vec3 s = shared_scale[j];
            float p = shared_phi[j];
            float max_s = fmaxf(s.x, fmaxf(s.y, s.z));
            
            // 你的自定义权重公式
            float weight = expf(max_s / d) + p / (d * d) + p * p;

            if (inside) {
                float val = alpha * weight;
                for (int ch = 0; ch < CHANNELS; ch++) {
                    atomicAdd(&s_color[ch][local_id], features[shared_id[j] * CHANNELS + ch] * val);
                }
                atomicAdd(&s_w_fg[local_id], val);
                atomicAdd(&s_Ts[local_id], logf(fmaxf(1.0f - alpha, 1e-6f)));
            }
        }
        block.sync(); // 处理完本批次，准备搬运下一批
    }

    // 6. 写回全局显存 (Coalesced Write)
    if (inside)
    {
        uint32_t pix_id = W * pix.y + pix.x;
        for (int ch = 0; ch < CHANNELS; ch++) {
            out_color[ch * W * H + pix_id] = s_color[ch][local_id];
        }
        w_fg[pix_id] = s_w_fg[local_id];
        Ts[pix_id] = s_Ts[local_id];
    }
}
...
// 			for (int ch = 0; ch < CHANNELS; ch++)
// 				{
// 				    C[ch] += features[collected_id[j] * CHANNELS + ch] * alpha * weight ;
//                     w_fg_c[ch] +=  alpha * weight;
// 				}
//             if(weight_max<alpha * weight)
// 			{
// 				weight_max=alpha * weight;
	...
	)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            weight = fminf(fmaxf(0.0f, weight), 0.999f);
			...
			float depth_ray = (-b/2/a)/glm::length(ray_direction);
...
// 		accum_alpha[pix_id] = T;
        ...
        out_depth[pix_id] = D;
        accum_alpha[pix_id] = weight_accum;
