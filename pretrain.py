...
from gaussian_renderer import render_impori, render_depth
import sys
from scene import Scene, TeaGaussianModel
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    gaussians = TeaGaussianModel(sh_degree=0)
    gaussians.vq_enabled = False
    gaussians.net_enabled = False 
    gaussians.opacity_theta_phi_nn=None
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        while network_gui.conn != None:
            try:
                ...
                if custom_cam != None:
                    net_image = render_impori(custom_cam, gaussians, pipe, background, scaling_modifer)["render"]
        ...
        render_pkg = render_impori(viewpoint_cam, gaussians, pipe, background)
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_impori, (pipe, background))
            ...
            if iteration == simp_iteration1:
                ...
                for view in views:
                    # print(idx)
                    render_pkg = render_impori(view, gaussians, pipe, background)
